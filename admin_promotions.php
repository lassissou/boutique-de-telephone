...
// Récupération des produits pour le champ sélection
try {
    $produits = $connexion->query("SELECT id FROM produits ORDER BY id DESC")->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_promotion'])) {
    ...
    if (!empty($nom_promotion) && !empty($type_reduction) && !empty($valeur_reduction) && !empty($date_debut) && !empty($date_fin) && !empty($produit_id)) {
        try {
            $requete = $connexion->prepare("
                INSERT INTO promotions (nom, type_reduction, valeur_reduction, date_debut, date_fin, produit_id)
                VALUES (:nom, :type_reduction, :valeur_reduction, :date_debut, :date_fin, :produit_id)
            ");
            $requete->execute([
                ':nom' => $nom_promotion,
